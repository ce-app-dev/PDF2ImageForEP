 ...
 * カスタムRadioGroup
 * 
 * 出力設定ダイアログの、出力サイズ指定のレイアウトにて、ラジオボタンの横に入力フォームを配置するため、以下から拝借しました
 * （通常のRadioGroupは直接の子要素のみグループ化するため、孫要素（RadioGroup-LinearLayout-RadioButtonのようなボタン）を同一グループにしてくれないので）
 * https://github.com/maripo/MaripoWidgets/blob/master/src/maripo/android/widget/DeepRadioGroup.java
 * 
 */
